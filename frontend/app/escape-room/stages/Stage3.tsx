...
const Stage3 = ({ onSuccess, onCancel }: Stage3Props) => {
      ...
      {mode === 'quiz' && (
        <div className="card er-border er-surface" style={{ maxWidth: '780px', width: '100%' }}>
          <div className="card-header d-flex justify-content-between align-items-center">
            <h5 className="mb-0" style={{ color: 'var(--text-primary)' }}>Stage 3 Quiz</h5>
            <span className="badge" style={{ backgroundColor: '#dc3545', color: 'white', border: '1px solid black' }}>Map Insight</span>
          </div>
          <div className="card-body">
            <p className="mb-3" style={{ color: 'var(--text-primary)' }}>What does finding another map usually indicate in treasure hunts?</p>
            <div className="list-group mb-3">
              {[
                { id: 'a', text: 'You reached the final location' },
                { id: 'b', text: 'There’s a clue leading to the next place' },
                { id: 'c', text: 'There is no treasure anywhere' }
              ].map(opt => (
